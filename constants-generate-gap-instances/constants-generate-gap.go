...
func main() {
	...
	strictString := ""
	if initVector.Strict {
		strictString = "_s"
	}
	fmt.Println("path-to-here/constants-generate-gap-instances/constants-instances/gap-a5n"+strconv.Itoa(initVector.N)+"m"+ strconv.Itoa(initVector.M)+"-"+strconv.Itoa(initVector.Seed)+strictString+"\");;runTook;")
	fmt.Println("Read(\"path-to-here/constants-generate-gap-instances/constants-instances/gap-a5n"+strconv.Itoa(initVector.N)+"m"+ strconv.Itoa(initVector.M)+"-"+strconv.Itoa(initVector.Seed)+"_u"+strictString+"\");;runTook;")
}
...
func initializeInitVector() () {
	...
	strictPtr := flag.Bool("strict", false, "strict sampling")
...
func generateInstance() {
	...
	constraints := generateConstraints(initVector.N, initVector.M, initVector.Strict)
	...
	plantedName := getFilename("constants-instances/gap-a5", initVector.N, initVector.M, initVector.Seed, initVector.Strict, true, initVector.X60)
	uplantedName := getFilename("constants-instances/gap-a5", initVector.N, initVector.M, initVector.Seed, initVector.Strict, false, initVector.X60)
...
func computeReverseRightHalf(constraints [][]int, assignment [][]int) [][]int {
	m := len(constraints)	
	reverseRightHalf := make([][]int, m)

	a := []int{4, 0, 1, 2, 3} // generator 1
	b := []int{1, 0, 3, 2, 4} // generator +/-2
	A := []int{1, 2, 3, 4, 0} // generator -1
	
	for i := 0; i < m; i++ {
		perm := []int{0,1,2,3,4}
		for j:=0;j<len(constraints[i]);j++ {
			switch constraints[i][j] {
				case -2: // b=B
...
func generateConstraints(n int, m int, strict bool) [][]int {
	constraints := make([][]int, 0, m)
	for i:= 0; i<m; i++ {
		//start with 'constant'
		currentConstraint := wordFromPermutation(randomEven5Permutation())
		for j := 1; j<arity; j++ {
			if j == 1 {
				currentConstraint = append(currentConstraint, nrA5Generators + 1 + rand.Intn(n)) // no duplicate checking needed for first part, so it is generated here
				currentConstraint = append(currentConstraint, wordFromPermutation(randomEven5Permutation())...) // constant between first and second
			}
			
			currentConstraint = append(currentConstraint, nrA5Generators + 1 + rand.Intn(n))
			
			acceptable := isAllDifferent(currentConstraint) && (!strict || !isDuplicateStrict(constraints, currentConstraint))
			
			if (!acceptable) {
				j = 0 //will be incremented to 1, started from beginning
				currentConstraint = wordFromPermutation(randomEven5Permutation()) //reset to 'only constant'
			} else {
				currentConstraint = append(currentConstraint, wordFromPermutation(randomEven5Permutation())...) //add constant after variable in the end
			}
		}
...
func isAllDifferent(constraintToCheck []int) bool {
	for i := 0; i<len(constraintToCheck); i++ {
		if constraintToCheck[i] <= nrA5Generators && constraintToCheck[i] >= (-1)*nrA5Generators { // don't compare constants
			continue
		}
		for j := 0; j<len(constraintToCheck); j++ {
			if constraintToCheck[j] <= nrA5Generators && constraintToCheck[j] >= (-1)*nrA5Generators { // don't compare constants
				continue
			}
...
func isDuplicateStrict(constraints [][]int, constraintToCheck []int,) bool { 
// Set notion: no more than 1 variable overlap between two constraints
	...
	for _, cons := range constraints {
		overlap := 0
		for i := 0; !(overlap > 1) && i<len(cons); i++ {
			if cons[i] <= nrA5Generators && cons[i] >= (-1)*nrA5Generators { // don't compare constants
				continue
			}
			for j := 0; j<len(constraintToCheck); j++ {
				if constraintToCheck[j] <= nrA5Generators && constraintToCheck[j] >= (-1)*nrA5Generators  { // don't compare constants
...
func writeInstanceFiles(constraints [][]int, reverseRightHalf [][]int, randomRightHalf [][]int, n int, plantedFileName string, unplantedFileName string) {
	...
	for i,cons := range constraints {
		...
		for j:=0;j<len(cons);j++ {
			...
			if constraints[i][j] < 0 {
				constraintBuilder.WriteString(strconv.Itoa((-1)*constraints[i][j]))
				unplantedBuilder.WriteString(strconv.Itoa((-1)*constraints[i][j]))
			...
			if !(j==len(cons)-1 && len(reverseRightHalf[i]) == 0) { // only if this is not identity
				constraintBuilder.WriteString("*")
			}
			if !(j==len(cons)-1 && len(randomRightHalf[i]) == 0) { // only if this is not identity
				unplantedBuilder.WriteString("*")
			}
		}
...
func getFilename(base string, n int, m int, seed int, strict bool, planted bool, x60 bool) string {
